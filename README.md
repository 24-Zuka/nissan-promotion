...
```bash
npm test               # shared(UT) + api(IT) + web(UT) のテスト
npm run typecheck      # 全パッケージの型チェック
npm run build          # shared → api → web をビルド
npm run lint           # TypeScript / React の静的解析
npm run e2e            # Chromium（PC / iPhone相当幅）の主要操作テスト
```
...
- 設定 → **データ管理**から、全業務データのJSONバックアップと顧客一覧CSVを保存できます。
- JSON復元は静的モード限定です。復元前に現在データを自動保存し、検証済みデータだけを一括復元します。
- PIN・認証トークン・Google認証情報・同期キューはバックアップへ含めません。
- タスク完了後は8秒間「元に戻す」が表示されます。
- 通知は同じタスクを同じ日に重複表示せず、アプリを開いた時に期限を確認します。
- 「長期セッション」OFFはブラウザ終了まで、ONは次回起動時もログイン状態を保持します。
- 画面テーマは設定から「端末設定・ライト・ダーク」を選択でき、選択内容はこの端末に保存されます。
